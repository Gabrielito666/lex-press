...
/**
 * Atributos de texto visible (alt, title, placeholder, aria-label) que nunca
 * se reescriben: su valor es texto leído por el usuario, no una URL de asset.
 * @type {Set<string>}
 */
...
const escapeAssets = (baseUrl, html) =>
{
	...
	// baseUrl normalizado: sin slashes sobrantes y SIEMPRE con "/" inicial
	// (si queda vacío se deja "" para no duplicar el slash del src).
	const trimmedBaseUrl = baseUrl.replace(/^\/+|\/+$/g, "");
	const normalizedBaseUrl = trimmedBaseUrl ? "/" + trimmedBaseUrl : "";
	...
	$("*").each((_, el) =>
	{
		...
		{
			if(typeof value !== "string" || TEXT_ATTRS.has(name))
			{
				continue;
			}
