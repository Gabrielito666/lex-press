...
/**
 * @import {LexpressDevApp, LexpressFn} from "#lib/lex-press-dev"
 */
...
const lexpress = Object.assign(/**@param {{ tag?: string }} [options] @returns {LexpressDevApp}*/(options) =>
{
	...
	app.use((req, res, next) =>
	{
		...
		// LIMITACIÓN: init() se ejecuta una sola vez con la baseUrl del primer
		// request que toca la app. Si la misma instancia se monta en dos bases
		// (app1.use(app2) y app1.use("/app-2", app2)), el mount de raíz recibe
		// los requests primero y el árbol queda escapado con "" — el html bajo
		// /app-2 se sirve con /__assets/ sin el prefijo de la base. Una app de
		// producción debe montarse en UNA sola base.
