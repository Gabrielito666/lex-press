 ...
 * decodifica el URI, rechaza path traversal (segmento ".." exacto delimitado
 * por inicio, "/" o "\") y colapsa el trailing slash (ej: "/ruta1/" => "/ruta1").
 * Devuelve null si la ruta es inválida (decode fallido o traversal).
...
const normalizeRoute = (pathname) =>
{
	...
	// ".." como segmento exacto (rodeado de inicio, "/" o "\") es path traversal
	// y se rechaza SIEMPRE; nombres legítimos como "v1..2" contienen ".." pero
	// no son traversal: el regex exige el delimitador para matchear.
	if(/(^|[\/\\])\.\.($|[\/\\])/.test(decoded))
	{
		return null;
	}
